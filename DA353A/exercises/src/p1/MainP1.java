...
public class MainP1 {
    ...
    private static boolean ascending(double[] arr, double min, double max) {
    	if(arr==null || arr.length==0)
    		return false;
        boolean ok = arr[0]==min && arr[arr.length-1]==max;
        for(int i=0; i<arr.length-1 && ok; i++) {
            ok = arr[i]<=arr[i+1];
        }
        return ok;
    }
    public static void testSearchSort() {
        ...
        Sorting.sort(arr);
        test(ascending(arr,50,79),"Sorting.sort(double[])","Sorting.sort(double[])");
    }
