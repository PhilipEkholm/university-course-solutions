...
err_code lcdWrite(uint8_t byte, bool type)		/* writes a byte to the LCD display */
/*
...
err_code lcdClearDisplay(void)
{
	...
	lcdWrite(LCD_REG_ENTRY_MODE, LOW);		/* Entry mode set: move cursor right, no display shift */
	delayMicroseconds(100);
	lcdWrite(LCD_REG_CURSOR_ON ,LOW);		/* Display on, cursor on, blinking on */
	lcdWrite(LCD_REG_DISPLAY_CLR, LOW);     /* Clear display */
